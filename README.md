...
Posts run for 30 days from creation (`expires_at`, set server-side — never client-supplied) and a coach can end one early. Browsing (`/roster-posts`) is open to any signed-in adult, parent or coach — unlike the player pool, a roster post isn't player data, so it doesn't need the verified-coach gate. The RLS policy simply hides expired posts from everyone except their own coach and admins, who can still see and manage them after expiry.
...
- **Rich video link previews** — richness without hosting anything. `src/lib/oembed/server.ts` asks the linked provider's own oEmbed endpoint (direct for YouTube/Vimeo, spec auto-discovery for anything else, e.g. Hudl/Veo) for a thumbnail and title; `<VideoLinkPreview>` renders that as a thumbnail-plus-play-button card on the coach-facing player detail view (`/search/[id]`). Every failure mode — unknown provider, network error, timeout, missing thumbnail — falls back to the original plain link. We still never fetch, proxy, or store the video itself, only a thumbnail URL the provider already serves publicly.
